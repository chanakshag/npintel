...
export default function BomIntel() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    toast.success("You're on the waitlist!", { description: "We'll be in touch when BOM Intel ships." });
    setEmail("");
  };
  return (
    <AppLayout title="BOM Intel" description="Coming soon">
      <div className="mx-auto flex max-w-xl items-center justify-center py-12">
        <Card className="w-full border-border/60 p-10 text-center">
          ...
          <h2 className="mt-6 text-2xl font-bold text-navy">BOM Intel</h2>
          <p className="mt-2 text-sm text-slate-600">Bill of materials intelligence & component risk monitoring</p>
